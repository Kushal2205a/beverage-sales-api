...
def clean_sales_data(df):
    ...
    for state in df['State'].unique():
        ...
        state_df_resampled['lag_1'] = state_df_resampled['Total'].shift(1)
        state_df_resampled['lag_7'] = state_df_resampled['Total'].shift(7)
        state_df_resampled['lag_30'] = state_df_resampled['Total'].shift(30)
        

        state_df_resampled['rolling_mean_4'] = state_df_resampled['Total'].shift(1).rolling(window=4).mean()
        state_df_resampled['rolling_std_4'] = state_df_resampled['Total'].shift(1).rolling(window=4).std()
